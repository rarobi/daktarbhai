...
@section('main-content')
    ...
    <h3 class="phr-title">{{__('phr.bmi.list_title')}}</h3>
        ...
        <div class="table-responsive phr-table">
            <table id="example" class="mdl-data-table" width="100%" cellspacing="0">
                ...
                <tr>
                    ...
                    <th>{{__('phr.bmi.table.title.height')}}</th>
                    <th>{{__('phr.bmi.table.title.weight')}}</th>
                    <th>{{__('phr.bmi.table.title.bmi_value')}}</th>
                ...
                @foreach($phrLists as $list)
                     <tr>
                        <td><a href="{!! route('frontend.phr.show', ['phr' => 'bmi', 'id'=> $list->id]) !!}">
                                {!! \Carbon\Carbon::parse($list->report_datetime)->format('F j, Y') !!}
                            </a>
                        </td>
                        <td>{{ $list->height or '' }}</td>
                        <td>{{ $list->weight or '' }} {{ $list->unit->weight }}</td>
                        <td>
                            <span  @if (($list->bmi_status) =='above') class="above"
                                   @elseif(($list->bmi_status) =='normal') class="normal"
                                   @else  class="below" @endif>
                                {{ $list->bmi or '' }} ({{ ucwords($list->bmi_status) }})
                            </span>
                        </td>
                        <td>
                            <div class="actions">
