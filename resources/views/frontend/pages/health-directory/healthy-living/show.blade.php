...
@section('title', $healthy_living->name . ' - Daktarbhai')
                        ...
                        <div class="hospital-info-area margin-bottom-small">
                            <div class="col-md-5 padding-null">
                                <div class="doc-images">
                                    <a href="#"><img src="{!! (isset($healthy_living) && $healthy_living->image_path !=null) ? $healthy_living->image_path : asset('assets/img/directory/healthy-living-directory.png') !!}" class="img-responsive img-center"></a>
                                </div>
                            </div>
                            <div class="col-md-7 padding-10">
                                <h2 class="hos-name">{!! $healthy_living->name or '' !!}</h2>
                                <!-- <p class="designation">BDS Consultant, Prime General Hospital</p> -->
                                <div class="address-wrap">
                                    <div class="address-left">
                                        @if(isset($healthy_living->address))
{{--                                            <p class="location"><i class="ion-ios-location-outline"></i> <span>{{__('find_hospital.address')}}</span><br>--}}
                                            <p class="location"><i><img src="{!! asset('assets/img/directory-location.png') !!}" alt=""></i>
                                                {!! $healthy_living->address or '' !!}
                                            </p>
                                        @endif
                                        @if(isset($healthy_living->email) && $healthy_living->email != null)
{{--                                            <p class="location"><i class="ion-ios-email-outline"></i> <span>{{__('find_hospital.mail')}}</span><br>--}}
                                            <p class="location"><i class="ion-ios-email-outline"></i>
                                                {!! $healthy_living->email or '' !!}
                                            </p>
                                        @endif

                                        @if(! isEmptyOrNull($healthy_living->landphone ) || ! isEmptyOrNull($healthy_living->landphone_2) )
                                            <p class="location">
{{--                                                <i class="ion-ios-telephone-outline"></i>--}}
{{--                                                <span>{{__('find_hospital.phone')}}</span><br>--}}
                                                <i><img src="{!! asset('assets/img/directory-landphone.png') !!}" alt=""></i>
                                                <a data-tel="{!! $healthy_living->landphone or '' !!}">
                                                    {!! $healthy_living->landphone or '' !!}
                                                </a>
                                                @if(! isEmptyOrNull($healthy_living->landphone_2) ) ,
                                                <a data-tel="{!! $healthy_living->landphone_2 or '' !!}">
                                                    {!! $healthy_living->landphone_2 or '' !!}
                                                </a>
                                                @endif
                                            </p>
                                        @endif

                                        @if(! isEmptyOrNull($healthy_living->mobile) || ! isEmptyOrNull($healthy_living->mobile_2))
                                            <p class="location">
{{--                                                <i class="fa fa-mobile"></i>--}}
{{--                                                <span>{{__('find_hospital.mobile')}}</span><br>--}}
                                                <i><img src="{!! asset('assets/img/directory-mobile.png') !!}"></i>
                                                <a data-tel="{!! $healthy_living->mobile or '' !!}">
                                                    {!! $healthy_living->mobile or '' !!}
                                                </a>
                                                @if(! isEmptyOrNull($healthy_living->mobile_2) )
                                                    ,
                                                    <a data-tel="{!! $healthy_living->mobile_2 or '' !!}">
                                                        {!! $healthy_living->mobile_2 or '' !!}
                                                    </a>
                                                @endif
                                            </p>
                                        @endif
...
{{--                                                    </table>--}}
                                                    <div class="col-md-12">
                                                        <p class="text-left justi">{{__('find_hospital.msg_12')}}
                                                        </p>
                                                    </div>
...
{{--                                                    <a id="avail_discount" href="" data-modal="#modal" class="modal__trigger btn-alt small active margin-null">{{__('find_hospital.buttons.avail_discount')}}</a>--}}
                                ...
                                <div role="tabpanel" class="tab-pane" id="tab-three">
                                    {!! (isset($healthy_living->description) && $healthy_living->description !=null) ? $healthy_living->description  : __('find_hospital.isPanel_outlet')   !!}
                                </div>
                            </div>
                        </div>
...
@section('before-scripts-end')
    ...
    <script type="text/javascript">
        ...
        function initMap() {
            var latitude = '{!! $healthy_living->latitude  !!}';
            var longitude = '{!! $healthy_living->longitude !!}';

            var name = "{!! $healthy_living->name  !!}";
        ...
        $(document).ready(function () {
            ...
            $("#avail_discount").on('click',function () {
                /*$(this).disabled = true;*/
                var id = '{!! $healthy_living->id !!}';
                var type = 'healthy-living';
